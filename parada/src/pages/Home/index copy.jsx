...
function Home() {
 ...
 /* const [carro, setCarro] = useState([
  ...
  {carro.map((veiculos) => (
    <div key={veiculos.id} className="card-container">
      <div>
        <p>Placa: <span>{veiculos.placa}</span></p>
        <p>Marca: <span>{veiculos.marca}</span></p>
        <p>Cor: <span>{veiculos.cor}</span></p>
      </div>
      <button onClick={() => deleteUser(veiculos.id)}>
        <img src={Lixeira} alt="Excluir" width={24} />
      </button>
    </div>
  ))}
</div>
